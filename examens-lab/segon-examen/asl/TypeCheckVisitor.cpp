...
antlrcpp::Any TypeCheckVisitor::visitFilter(AslParser::FilterContext *ctx) {
  ...
  TypesMgr::TypeId t1 = getTypeDecor(ctx->expr(0));
  TypesMgr::TypeId t2 = getTypeDecor(ctx->expr(1));
  TypesMgr::TypeId t3 = getTypeDecor(ctx->expr(2));


  bool thereIsError = false;

  TypesMgr::TypeId telem1 = Types.createErrorTy();
  if (not Types.isErrorTy(t1) and not Types.isArrayTy(t1))  //no es array
    thereIsError = true;
    //Errors.incompatibleOperator(ctx->op);
  else if ( not Types.isErrorTy(t1) )  {
    telem1 = Types.getArrayElemType(t1);

  }

  if (not Types.isErrorTy(t2) and not Types.isArrayTy(t2))  //no es array
    thereIsError = true;
    //Errors.incompatibleOperator(ctx->op);
  else if (not Types.isErrorTy(t2)) {
    TypesMgr::TypeId elem = Types.getArrayElemType(t2);

    if ( (not Types.isErrorTy(elem) and not Types.isBooleanTy(elem)) ){
      thereIsError = true;
      //Errors.incompatibleOperator(ctx->op);
    }
    if (Types.getArraySize(t1) != Types.getArraySize(t2) ) {
  ...
  if (not Types.isFunctionTy(t3) and not Types.isErrorTy(t3)) {   //ID no es funcion
    thereIsError = true;
    //Errors.incompatibleOperator(ctx->op);
  }
  else if (not Types.isErrorTy(t3)) {
    //int numParam = Types.getNumOfParameters(t3);
    if (Types.getNumOfParameters(t3) != 1){
      thereIsError = true;
      //Errors.incompatibleOperator(ctx->op);
    }
    else if (not Types.isErrorTy(telem1) and (Types.getParameterType(t3,0) != telem1)){ //mejor incompatible...
      thereIsError = true;
      //Errors.incompatibleOperator(ctx->op);
    }
    //std::vector<TypesMgr::TypeId> lParamsTy = Types.getFuncParamsTypes(t3);

    TypesMgr::TypeId tr = Types.getFuncReturnType(t3);
    if (not Types.isBooleanTy(tr)){
      thereIsError = true;
      //Errors.incompatibleOperator(ctx->op);
    }
  }
  ...
  TypesMgr::TypeId tFinal = Types.createIntegerTy();
...
antlrcpp::Any TypeCheckVisitor::visitSum(AslParser::SumContext *ctx) {
  ...
  for (size_t i = 0; i < (ctx->expr()).size(); ++i) {  //visitamo elementos
    ...
    if ( (not Types.isErrorTy(elem) and not Types.isNumericTy(elem)) ){
      //Errors.incompatibleOperator(ctx->op);
      isError = true;
    }
    ...
    if (Types.isNumericTy(elem)) {
      ...
      if (Types.isIntegerTy(elem)) isInt = true;
      else isFloat = true; //!!!
    }
  }

  //if (isChar) tFinal = Types.createCharacterTy();
  if (isInt) tFinal = Types.createIntegerTy();
